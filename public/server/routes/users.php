...
/**
 * Returns user's pending messages on a certain channel
 */
$app->get('/users/:uid/channel/:cid/pending/', function ($uid,$cid) use ($app, $req, $res) {
  ...
  if(!Container_channels::checkChannelUser($cid,$uid)){
    $res->status(404);
    $res['Content-Type'] = 'application/json; charset=utf-8';
    $res->body('{ "error": "User does not belong to the channel" }');
    return;
  }
  // store that user is alive
  Container_users::setIsAlive($uid);
  ...
  $res->status(200);
  $res['Content-Type'] = 'application/json; charset=utf-8';
  $res->body(Container_users::getUserMsgs($uid, true,$cid));
});
