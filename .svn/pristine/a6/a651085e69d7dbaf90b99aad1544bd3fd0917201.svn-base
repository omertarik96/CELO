...
class Container_users {
  ...
  static public function getUserData($uid, $field = null, $injson = false) {
    ...
    if ($field) {
      ...
      $data = (array) $data;
      $data['channels']=self::getUserChannels($uid);
  ...
  static public function setCloseFlag($uid) {
    ...
    if (!file_exists($close_file)) {
        $userInfo=Container_users::getUserData($uid);
        Container_messages::postMsgToUserChannels($uid,$userInfo->name. " Closed there window","info");
      touch($close_file);
  ...
  static public function getUserMsgs($uid, $injson = false,$cid=null) {
    $umdir = self::getDir().'/'.$uid.'/messages';
    $msgs = array();
    foreach (scandir($umdir) as $value) {
      if ($value === '.' || $value === '..') continue;
      $messageContent=file_get_contents($umdir.'/'.$value);

      if($cid!=null){
        $message=json_decode($messageContent);
        if(isset($message->recipient)){
          if(preg_match('/channel\|(?<name>.*)/',$message->recipient,$matches)){
             if($cid!=$matches['name']){
               continue;
             }
          }
  ...
  static public function leaveChannel($uid, $cid) {
    $ret = true;
      ...
      if(Container_channels_op::isOp($cid,$uid)) {
          Container_channels_op::rmOp($cid, $uid);
      }
    // clean user data
    $ucpath  = self::getDir().'/'.$uid.'/channels/'.$cid;
    ...
    if(count(Container_channels::getChannelUsers($cid))==0){
        $chanelFile=Container_channels::getChannelDir($cid);
      if(file_exists($chanelFile)){
        rrmdir($chanelFile);
      }
    }
    return $ret;
